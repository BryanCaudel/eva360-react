...
export default function EvaluacionesScreen({ navigation }) {
  ...
  const renderItem = ({ item }) => {
    const { codigo, evaluado_nombre, por_area = {} } = item;
    ...
    return (
      <View style={{
        ...
        {entries.length === 0 ? (
          <Text style={{ color: "#6b7280" }}>Sin respuestas aún.</Text>
        ) : (
  ...
  return (
    <View style={{ flex: 1, padding: 16 }}>
      <Text style={{ fontSize: 22, fontWeight: "800", marginBottom: 12 }}>Evaluaciones (promedio por sesión)</Text>
      ...
      {loading ? <ActivityIndicator /> :
       ...
       <FlatList data={items} keyExtractor={(it) => String(it.sesion_id)} renderItem={renderItem}
